...
import '../painter/chart_engine.dart';
...
mixin TimeDataProcessor {
  ...
  void _countDays(List<DateTimeRange> dataList) {
    ...
    _dayCount = firstDateTime.differenceDateInDay(lastDateTime) + 2;
  }
  ...
  _TimePair? _getPivotHoursFrom(List<DateTimeRange> dataList) {
    ...
    for (int i = 1; i < len; ++i) {
      if (resultPair.startTime < rangeList[i].startTime) {}
    ...
    return _TimePair(resultPair.startTime, resultPair.endTime);
  }
